...
export default function CreateCard({ initialData, onSubmit, isEditing = false, externalLoading = false }) {
  ...
  const handleSizeChange = (size) => {
    if (isEditing && !window.confirm("Changing size will clear the current grid. Continue?")) return;
    setGridSize(size);
      ...
      <div className="max-w-[1600px] mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center bg-slate-900/40 border border-bingo-red p-6 rounded-3xl shadow-xl shadow-red-900/10">
          <div className="flex items-center gap-4">
            <Link to="/admin/cards" className="p-2 hover:bg-slate-800 rounded-xl text-bingo-red transition-colors">
              <ChevronLeft />
            </Link>
            <h1 className="text-2xl font-black uppercase tracking-tighter">
              {isEditing ? `Editing: ${initialData?.title}` : "Create New Bingo Card"}
            </h1>
          </div>
          <button
            onClick={handleSave}
          ...
          >
            <Save size={20} /> {isProcessing ? "Saving..." : isEditing ? "Update Card" : "Publish Card"}
          </button>
        </div>
        ...
        <div className="grid grid-cols-12 gap-8">
          ...
          <div className="col-span-3 space-y-4">
            <div className="bg-slate-900/40 border border-bingo-red p-6 rounded-3xl space-y-6">
              ...
              <div>
                <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Purchase Price (€)</label>
                <input
                  min="1"
                  value={price}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    value={prizePerLine}
                    onChange={(e) => handlePrizeChange(setPrizePerLine, e.target.value)}
                ...
                <div>
                  ...
                  <input
                    value={fullPrize}
                    onChange={(e) => handlePrizeChange(setFullPrize, e.target.value)}
            ...
            <div className="p-4 border border-bingo-red bg-red-500/5 rounded-2xl flex gap-3 text-white shadow-lg">
              ...
              <p className="text-[10px] font-semibold uppercase leading-tight">
                Drag events from the library and drop them into the grid slots to organize your card.
              </p>
            </div>
          </div>
            ...
            >
              {gridEvents.map((slot, idx) => (
                ...
                >
                  {slot ? (
                    <>
                      <span className="text-[8px] font-black text-bingo-red uppercase tracking-tighter">{slot.sport}</span>
                      <p className="text-[10px] font-bold leading-tight mt-1 truncate w-full px-1">{slot.team1}</p>
                      <span className="text-[8px] text-slate-600 font-black">VS</span>
                      <p className="text-[10px] font-bold leading-tight truncate w-full px-1">{slot.team2}</p>
                      <button
                    ...
                    <Plus size={24} className="text-slate-800 group-hover:text-slate-600 transition-colors" />
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="col-span-4 bg-slate-900/40 border border-bingo-red rounded-3xl p-6 h-[75vh] flex flex-col shadow-xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-black uppercase tracking-tighter">Event Library</h2>
            ...
            <div className="overflow-y-auto flex-grow space-y-3 pr-2 custom-scrollbar">
              {filteredEvents?.map((event) => (
                ...
                >
                  ...
                  <p className="font-bold text-sm group-hover:text-white transition-colors">{event.team1} vs {event.team2}</p>
                  <p className="text-[10px] text-slate-500 font-medium">{event.prediction}</p>
                </div>
              ))}
              {filteredEvents?.length === 0 && (
                <div className="text-center py-10 text-slate-600 font-bold uppercase text-xs">No events found</div>
              )}
            </div>
          </div>
        </div>
