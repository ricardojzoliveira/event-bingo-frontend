...
export default function UserManagementPage() {
    ...
    const handleUpdateStatus = (user, newStatus) => {
        const actionText = newStatus === "ACTIVE" ? "ativar" :
            newStatus === "SUSPENDED" ? "suspender" : "alterar";

        if (window.confirm(`Are you sure you want to ${actionText} the user ${user.full_name}?`)) {
    ...
    const handleDeleteUser = (user) => {
        if (window.confirm(`WARNING: You are about to delete the user ${user.full_name}. Continue?`)) {
        ...
        <div className="min-h-screen bg-bingo-dark p-6 md:p-12 text-white">
            <div className="max-w-7xl mx-auto space-y-8">
                <div className="border border-bingo-red rounded-2xl p-8 bg-slate-900/20 flex items-center gap-6">
                    <Link to="/admin" className="p-3 hover:bg-slate-800 rounded-xl text-bingo-red border border-slate-800">
                        <ChevronLeft size={24} />
                    </Link>
                    <div>
                        <h1 className="text-3xl font-black uppercase tracking-tighter">Users Management</h1>
                        <p className="text-slate-500 font-bold uppercase text-sm">Manage users</p>
                    </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <StatMiniCard label="Total Users" value={stats.total} icon={Users} color="slate" />
                    <StatMiniCard label="Active Users" value={stats.active} icon={UserCheck} color="slate" />
                    <StatMiniCard label="Excluded" value={stats.Excluded} icon={UserMinus} color="slate" />
                    <StatMiniCard label="Deleted" value={stats.deleted} icon={UserX} color="red" />
                </div>
                            ...
                            {filteredUsers?.map((u) => {
                                ...
                                return (
                                    <tr key={u.id} className="hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="p-6 font-bold text-xs uppercase text-slate-400">{u.username}</td>
                                        <td className="p-6 font-bold text-xs uppercase text-slate-400">{u.role}</td>
                                        <td className="p-6">
                                            <span className={`px-2 py-1 rounded text-[10px] font-black uppercase ${getStatusColor(u.status)}`}>
                                                {u.status}
                                            </span>
                                        </td>
                                        ...
                                        <td className="p-6 text-right">
                                            {canPerformAction ? (
                                                <>
                                                    ...
                                                    {u.status === "SUSPENDED" ? (
                                                        <button onClick={() => handleUpdateStatus(u, "ACTIVE")} className="p-2 rounded-lg transition-all hover:bg-green-500/10 hover:text-green-500 text-slate-500">
                                                            <UserCheck size={16} />
                                                        </button>
                                                    ) : (
                                                        <button onClick={() => handleUpdateStatus(u, "SUSPENDED")} className="p-2 rounded-lg transition-all hover:bg-orange-500/10 hover:text-orange-500 text-slate-500">
                                                            <Ban size={16} />
                                                        </button>
                                                    )}
                                                    <button onClick={() => handleDeleteUser(u)} className="p-2 rounded-lg transition-all hover:bg-red-600/10 hover:text-red-600 text-slate-500">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </>
                                            ) : (
                                                <span className="text-slate-700 text-xs italic uppercase">Read Only</span>
                                            )}
                                        </td>
                                    </tr>
                                );
